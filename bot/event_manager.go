...
type EventManager interface {
	...
	HandleHTTPEvent(respondFunc httpserver.RespondFunc, event httpserver.EventInteractionCreate, original []byte)
...
type HTTPServerEventHandler interface {
	HandleHTTPEvent(client Client, respondFunc httpserver.RespondFunc, event httpserver.EventInteractionCreate, original []byte)
}
...
func (e *eventManagerImpl) HandleHTTPEvent(respondFunc httpserver.RespondFunc, event httpserver.EventInteractionCreate, original []byte) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.httpServerHandler.HandleHTTPEvent(e.client, respondFunc, event, original)
}
